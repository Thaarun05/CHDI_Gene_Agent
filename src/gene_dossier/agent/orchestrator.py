...
from .comparison import build_hd_modifier_matrix, hd_modifier_requirements
from .models import (
...
class RequirementReadinessRow:
    ...
    qualifying_dossier_run_ids: list[str]
    qualifying_tool_run_ids: list[str]
...
class ScientificAgentService:
    ...
    def __init__(
        ...
        *,
        ...
        self.accepted_baselines = {gene.upper(): run_id for gene, run_id in accepted_baselines.items()}
    ...
    def _manifest(run: DossierRun) -> dict[str, Any] | None:
        ...
        if manifest.get("schema_version") != ACQUISITION_MANIFEST_SCHEMA:
            return None
        if manifest.get("status") not in {"completed", "completed_with_errors"} or manifest.get("successful") is not True:
            return None
    ...
    def _latest_generated_run(self, gene: str) -> str | None:
        ...
        for report in reports:
            if report.gene_symbol.strip().upper() != gene or report.status.strip().lower() != "completed":
                continue
    ...
    def _known_genes(self) -> list[str]:
        ...
        return sorted(set(self.accepted_baselines) | {str(gene).strip().upper() for gene in stored if str(gene).strip()})
    ...
    def _refresh_universe(universe: AgentEvidenceUniverse) -> None:
        ...
        universe.tool_run_ids = _unique([*universe.reused_tool_run_ids, *universe.created_tool_run_ids])
        ...
        if universe.base_evidence_run_id and universe.evidence_universe.startswith("accepted_demo"):
            universe.evidence_universe = "accepted_demo_with_tool_overlay" if has_tools else "accepted_demo"
        elif universe.base_evidence_run_id and universe.evidence_universe.startswith("latest_generated"):
            universe.evidence_universe = "latest_generated_with_tool_overlay" if has_tools else "latest_generated"
        elif universe.base_evidence_run_id:
            universe.evidence_universe = "explicit_run_with_tool_overlay" if has_tools else "explicit_run"
        else:
    ...
    def _resolve_initial_universe(
        ...
        elif request.evidence_selection is not EvidenceSelection.explicit_only and gene in self.accepted_baselines:
    ...
    def _retrieve_requirement(
        ...
        *,
        ...
        eligible = qualifying_records(records, scoped_requirement)
    ...
    @staticmethod
    def _assessment(retrieval: RequirementRetrieval) -> EvidenceRequirementAssessment:
        requirement = retrieval.requirement
        count = len({hit.record.id for hit in retrieval.hits})
    ...
    def _find_reusable_runs(
        ...
        *,
        ...
        for run in runs:
            ...
            matching = needed_capabilities & manifest_capabilities
            if not matching:
                continue
            records = qualifying_records(self._load_run_records(gene, run.id), scoped)
            verified_matching = {
                capability
            ...
            capability_records = {
                ...
                if any(
                    record_matches_capability(record, capability)
            ...
            selected_record_ids.update(capability_records)
            excluded_run_ids.add(run.id)
            if len(selected_record_ids) >= requirement.minimum_support:
                break
        if require_threshold and len(selected_record_ids) < requirement.minimum_support:
            return []
        return selected
    ...
    def _tag_acquisition_run(
        ...
        *,
        ...
        with session_scope(self.engine) as session:
            run = get_dossier_run(session, run_id)
            if run is None or run.gene_symbol.strip().upper() != gene:
                raise ValueError("Persisted acquisition run could not be resolved for manifest tagging.")
    ...
    def _verified_capabilities(
        ...
        *,
        ...
        for capability in capabilities:
            if any(
                record.gene_symbol.upper() == gene
                and record_matches_capability(record, capability)
                for record in records
            ):
                verified.append(capability)
        if include_identity and any(
            record_matches_capability(record, CapabilityId.identity_function)
            for record in records
        ):
    ...
    def _execute_group(
        ...
        *,
        ...
        if self.read_only:
            raise RuntimeError("Controlled acquisition is disabled in read-only scientific-agent mode.")
        ...
        if bootstrap_identity and group.executor_kind == "section_bundle" and "1a" not in section_keys:
        ...
        try:
            ...
            activity.errors = [sanitize_credentials(str(error)) for error in getattr(result, "errors", [])]
            ...
            try:
                index = self.index_factory()
                activity.indexed_records = index.upsert_evidence(records) if index.available and records else 0
            except Exception as exc:  # noqa: BLE001
            ...
            try:
                ...
                logger.exception("acquisition manifest tagging failed for %s", activity.dossier_run_id)
                activity.errors.append(f"Acquisition reuse manifest was not persisted: {type(exc).__name__}")
        except Exception as exc:  # noqa: BLE001
    ...
    def _augment_hd_plan(self, plan: ScientificQuestionPlan) -> ScientificQuestionPlan:
        ...
        hd_rubric_covered_needs = {
            *rubric_needs,
            # General planner needs that are already represented by the
            # deterministic HD modifier lens dimensions.
            # They can still be retrieved if present in the selected universe,
            # but they must not create extra required acquisition.
        ...
        return plan.model_copy(update={"evidence_requirements": [*rubric, *extra]})
    ...
    def _select_final_records(retrievals: list[RequirementRetrieval]) -> list[EvidenceRecord]:
        ...
        while buckets and len(selected) < MAX_FINAL_RECORDS:
            ...
            for bucket in buckets:
                while bucket and bucket[0].record.id in seen:
                    bucket.pop(0)
    ...
    def _comparison_records(
        ...
        for retrieval in retrievals:
            if retrieval.requirement.id not in requirement_ids:
                continue
            for record in retrieval.qualifying_records:
                if record.id in seen:
                    continue
    ...
    def _overall_retrieval_metadata(retrievals: list[RequirementRetrieval]) -> tuple[str, str]:
        ...
        method = "semantic" if "semantic" in methods else "keyword" if "keyword" in methods else "metadata" if "metadata" in methods else "abstain"
        backend = "local_minilm" if "local_minilm" in backends else "real" if "real" in backends else "unavailable"
    ...
    def _evidence_system(record: EvidenceRecord) -> str:
        ...
        organism = " ".join(
            str(item or "").lower()
            for item in (record.organism, record.species, record.value.get("organism"), record.value.get("species"))
        )
        ...
        if source_type == "genetic_database" or any(term in organism for term in ("human", "homo sapiens")):
            return "human"
        if source_type == "model_organism_database" or any(term in organism for term in ("mouse", "mus musculus", "rat", "drosophila", "zebrafish")):
            return "animal/model-organism"
        if assertion in {"perturbation", "chemical_interaction", "chemical_tool"} or any(term in text for term in ("cell", "in vitro", "cellular", "culture")):
            return "in-vitro/cellular"
        if source_type in {"structure_database", "interaction_database"} or any(term in text for term in ("predicted", "alphafold", "computational", "string")):
    ...
    def _claim_type(record: EvidenceRecord) -> str:
        assertion = str(getattr(record.assertion_type, "value", record.assertion_type)).lower()
        if assertion in {"variant_association", "disease_association", "ppi", "expression", "cell_type_expression"}:
            return "association"
        if assertion in {"protein_function", "pathway_membership", "protein_structure", "literature_summary", "orthology", "transcription_factor_association"}:
            return "mechanistic"
        if assertion in {"perturbation", "chemical_interaction", "chemical_tool", "knockout_phenotype"}:
    ...
    def _structured_gaps(self, assessments: list[EvidenceRequirementAssessment]) -> list[EvidenceGap]:
        return [
            EvidenceGap(
    ...
    def _citation_registry(self, records: list[EvidenceRecord]) -> list[CitationReference]:
        return [
            CitationReference(
                ordinal=index,
    ...
    def _evidence_categories(
        ...
        for assessment in sorted(
            ...
            systems = sorted({self._evidence_system(record) for record in matched}) or ["not specified"]
            claim_types = sorted({self._claim_type(record) for record in matched}) or ["not specified"]
            blocks.append(
                EvidenceCategoryBlock(
                    ...
                    citation_ordinals=[ordinal_by_id[record.id] for record in matched if record.id in ordinal_by_id],
                    source_names=sorted({record.source_name for record in matched}),
                    retrieval_timestamps=sorted(
                        {timestamps[record.api_run_id] for record in matched if record.api_run_id in timestamps}
                    ),
    ...
    def _source_attempts(self, universes: dict[str, AgentEvidenceUniverse]) -> list[SourceAttempt]:
        run_ids = _unique([run_id for universe in universes.values() for run_id in universe.dossier_run_ids])
        if not run_ids:
            return []
        attempts: list[SourceAttempt] = []
        with session_scope(self.engine) as session:
            api_rows = session.exec(select(ApiRunRow).where(ApiRunRow.dossier_run_id.in_(run_ids))).all()
            coverage_rows = session.exec(
                select(SourceCoverageResultRow).where(SourceCoverageResultRow.dossier_run_id.in_(run_ids))
            ).all()
            for row in api_rows:
                attempts.append(
                    SourceAttempt(
        ...
        return sorted(
            attempts,
            key=lambda item: (item.dossier_run_id, item.source_name, item.retrieved_at or ""),
        )
    ...
    def _readiness_rows(
        ...
        *,
        ...
        assessment_map = {
            (item.requirement_id, item.gene_symbol): item for item in assessments
        }
        ...
        for retrieval in retrievals:
            ...
            sources = _unique([
                source
                for spec in specs
                for source in (list(spec.sources) if spec.executor_kind == "source_workflow" else [])
            ])
            ...
            qualifying_run_ids = _unique([record.dossier_run_id for record in retrieval.qualifying_records])
            universe = universes[gene]
            qualifying_tool_ids = [
            ...
            rows.append(
                RequirementReadinessRow(
                    ...
                    required=requirement.required,
                    minimum_support=requirement.minimum_support,
                    qualifying_persisted_record_count=len({record.id for record in retrieval.qualifying_records}),
                    qualifying_dossier_run_ids=qualifying_run_ids,
                    qualifying_tool_run_ids=qualifying_tool_ids,
    ...
    def readiness_table(
        ...
        *,
        ...
        universes = {
            gene: self._resolve_initial_universe(gene, request)
            for gene in plan.entities.genes
        }
        if not request.refresh_if_available:
            for requirement in sorted(plan.evidence_requirements, key=lambda item: (not item.required, item.id)):
                for gene in requirement.genes:
                    universe = universes[gene]
        ...
        assessments = [self._assessment(item) for item in retrievals]
        return self._readiness_rows(retrievals=retrievals, assessments=assessments, universes=universes), universes

    @staticmethod
    def _recommendations(gaps: list[EvidenceGap], records: list[EvidenceRecord]) -> list[ExperimentRecommendation]:
        priority = {
        ...
        for gap in sorted(gaps, key=lambda item: (priority.get(item.evidence_need.value, 99), item.id)):
            ...
            if gap.evidence_need.value == "human_genetic_association":
                ...
                uncertainty = "Whether the target is tractable with interpretable perturbation tools."
                ...
                uncertainty = "Whether the target is present in the relevant tissue and cell context."
            else:
    ...
    def execute(self, request: ScientificAgentRequest) -> ScientificAgentResult:
        ...
        if self.read_only and request.allow_tool_acquisition:
            raise ValueError("Read-only scientific-agent mode requires allow_tool_acquisition=False.")
        ...
        if plan_result.status is AnswerStatus.out_of_scope:
            ...
            return ScientificAgentResult(
                ...
                summary=plan_result.message or "The request is outside supported biomedical research scope.",
                limitations=["No evidence acquisition or scientific synthesis was attempted."],
                evidence_gaps=[],
                agent_activity=["Planning classified the request as outside supported biomedical research scope."],
        ...
        if plan_result.plan is None:
            ...
            return ScientificAgentResult(
                ...
                question=question,
                context_gene=context_gene,
                summary=plan_result.message or "The scientific question could not be planned safely.",
        ...
        if len(plan.entities.genes) > MAX_GENES or len(plan.evidence_requirements) > MAX_REQUIREMENTS:
            timings["total_request"] = round(time.perf_counter() - request_start, 6)
            return ScientificAgentResult(
                ...
                evidence_gaps=["Narrow the question to at most 6 genes and 10 evidence requirements."],
        ...
        stage_start = time.perf_counter()
        initial_assessments = [self._assessment(item) for item in initial_retrievals]
        ...
        readiness_by_key = {
            (row.requirement_id, row.gene): row for row in readiness_rows
        }
        actionable: list[tuple[EvidenceRequirement, str]] = []
        for requirement in sorted(plan.evidence_requirements, key=lambda item: (not item.required, item.id)):
            for gene in requirement.genes:
                row = readiness_by_key[(requirement.id, gene)]
                if row.acquisition_needed or (
        ...
        for requirement, gene in actionable:
            ...
            if not request.refresh_if_available:
                ...
                if reusable:
                    ...
                    continue
            if not request.allow_tool_acquisition:
                continue
            candidates = acquisition_capabilities(requirement)
            if not candidates:
                continue
        ...
        if source_errors:
            ...
            grouped = {
                key: group for key, group in grouped.items() if group.executor_kind != "source_workflow"
            }

        execution_counts: dict[str, int] = defaultdict(int)
        ordered_groups = sorted(grouped.values(), key=lambda group: (group.executor_kind != "section_bundle", group.gene))
        ...
        for group in ordered_groups:
            ...
            if result.dossier_run_id and result.status.lower() != "failed":
        ...
        if tool_activity:
            ...
            activity.append(f"Acquisition stage: {invoked_count} execution(s), {reused_count} reused run(s).")
        else:
        ...
        stage_start = time.perf_counter()
        assessments = [self._assessment(item) for item in final_retrievals]
        ...
        status = AnswerStatus.answered if all_required_sufficient else AnswerStatus.insufficient_evidence
        ...
        citation_registry = self._citation_registry(selected_records)
        evidence_categories = self._evidence_categories(selected_records, assessments)
        dimensions: list[str] = []
        matrix = []
        if plan.analysis_lens == "hd_modifier_relevance":
            hd_requirements = plan.evidence_requirements[
                : len(hd_modifier_requirements(plan.entities.genes))
            ]
        ...
        retrieval_timestamps = sorted(
            {
                timestamp
                for block in evidence_categories
                for timestamp in block.retrieval_timestamps
            }
        )
        limitations = [
        ...
        if structured_gaps:
            limitations.append("Limited, missing, and unsupported optional evidence remains disclosed in evidenceGaps.")
        if not selected_records:
            summary = "Insufficient provenance-backed evidence is available in the selected universes."
...
def summarize_agent_result_runs(result: ScientificAgentResult) -> dict[str, Any]:
    """Summarize acquisition/reuse provenance from the real result contract."""
    created = _unique(
        [
            run_id
    ...
    return {
        "createdDossierRunIds": created,
        "reusedDossierRunIds": reused,
        "createdToolRunIds": tool_created or created,
        "reusedToolRunIds": tool_reused or reused,
        "sourceAttempts": [item.model_dump(mode="json") for item in result.source_attempts],
        "evidenceRecordCounts": {
            item.dossier_run_id or "unknown": item.evidence_records_persisted
            for item in result.tool_activity
            if item.dossier_run_id
        },
