"""Protected attempt layout and accepted gene pointers for Section 5a.

Layout::

    data/outputs/section_5a/
...
SECTION_5A_OUTPUT_DIRNAME = "section_5a"
...
class Section5aPaths:
    """Resolved directory layout for Section 5a outputs."""
...
def paths_for(output_root: str | Path) -> Section5aPaths:
    root = Path(output_root)
    if root.name != SECTION_5A_OUTPUT_DIRNAME:
        root = root / SECTION_5A_OUTPUT_DIRNAME
    paths = Section5aPaths(root=root)
    paths.ensure()
    return paths
...
def accept_gene_report(
    ...
    *,
    ...
    """Pin a gene attempt as the accepted Section 5a report."""
