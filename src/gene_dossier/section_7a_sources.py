"""Protected attempt layout and accepted gene pointers for Section 7a.

Layout::

    data/outputs/section_7a/
      attempts/{RUN_ID}/{GENE}/
      accepted/genes/{GENE}.json

A failed or partial retry must never destroy a previous successful attempt.
Accepted pointers are written only after post-render evaluation in the bundle.
"""
...
SECTION_7A_OUTPUT_DIRNAME = "section_7a"
...
class Section7aPaths:
    """Resolved directory layout for Section 7a outputs."""
...
def paths_for(output_root: str | Path) -> Section7aPaths:
    root = Path(output_root)
    if root.name != SECTION_7A_OUTPUT_DIRNAME:
        root = root / SECTION_7A_OUTPUT_DIRNAME
    paths = Section7aPaths(root=root)
    paths.ensure()
    return paths
...
def accept_gene_report(
    ...
    *,
    ...
    """Pin a gene attempt as the accepted Section 7a report."""
...
def load_accepted_gene(
    paths: Section7aPaths,
    *,
    gene_symbol: str,
) -> dict[str, Any] | None:
    """Return the accepted gene pointer payload, or ``None`` if missing/invalid."""
    pointer = paths.accepted_gene_pointer(gene_symbol)
