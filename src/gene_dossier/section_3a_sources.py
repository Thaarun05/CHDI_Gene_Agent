"""Protected attempt layout and accepted gene pointers for Section 3a.

Layout::

    data/outputs/section_3a/
...
SECTION_3A_OUTPUT_DIRNAME = "section_3a"
...
class Section3aPaths:
    """Resolved directory layout for Section 3a outputs."""
...
def paths_for(output_root: str | Path) -> Section3aPaths:
    root = Path(output_root)
    if root.name != SECTION_3A_OUTPUT_DIRNAME:
        root = root / SECTION_3A_OUTPUT_DIRNAME
    paths = Section3aPaths(root=root)
    paths.ensure()
    return paths
...
def accept_gene_report(
    ...
    *,
    ...
    """Pin a gene attempt as the accepted Section 3a report."""
