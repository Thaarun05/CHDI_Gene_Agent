"""Protected attempt layout and accepted gene pointers for Section 4a.

Layout::

    data/outputs/section_4a/
...
SECTION_4A_OUTPUT_DIRNAME = "section_4a"
...
class Section4aPaths:
    """Resolved directory layout for Section 4a outputs."""
...
def paths_for(output_root: str | Path) -> Section4aPaths:
    root = Path(output_root)
    if root.name != SECTION_4A_OUTPUT_DIRNAME:
        root = root / SECTION_4A_OUTPUT_DIRNAME
    paths = Section4aPaths(root=root)
    paths.ensure()
    return paths
...
def accept_gene_report(
    ...
    *,
    ...
    """Pin a gene attempt as the accepted Section 4a report."""
